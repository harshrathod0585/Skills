...
const fs = require('fs');
const { usagePath } = require('./policy');
...
for (const surface of ['proxy', 'agent']) {
  ...
  console.log(`    model     ${fmt(tally(picked, 'model'))}`);
  console.log(`    effort    ${fmt(tally(picked, 'effort'))}`);
  ...
  const moves = list.filter(r => r.changed && r.from && r.model && r.from !== r.model)
    .reduce((acc, r) => {
      const k = `${r.from} -> ${r.model}`;
      acc[k] = (acc[k] || 0) + 1;
      return acc;
    }, {});
  if (Object.keys(moves).length) console.log(`    moves     ${fmt(moves)}`);

  const effortOnly = list.filter(r => r.changed && r.from && r.from === r.model).length;
